...
Decomposition heuristics: dot_claude/skills/claude-md-improver/references/decomposition-heuristics.md
...
For each section, pick a fate using the heuristics in `dot_claude/skills/claude-md-improver/references/decomposition-heuristics.md`:
...
2. Write each new rule file with appropriate frontmatter (`paths:` if scoped)
3. Write `CLAUDE.local.md` if any extractions need it; check `.gitignore` and add an entry if missing
4. Stub each skill with a minimal `SKILL.md` (real workflow filled in later by the user — don't fabricate)
