...
description: >-
  ...
  program running in a session, write a file into a session over SSH, or set up
  zmx shell integration (prompt indicator, completions, fzf session picker, SSH
  config). Covers every zmx subcommand with v0.6.0 flag semantics and the
  behavior gotchas the README glosses over.
---
...
| `completions <shell>` | `c` | Emit a completion script (`bash`/`zsh`/`fish`) |
...
- **`run` needs a `$?`-capable shell.** It detects the command's exit code via
  a shell marker. If the session's shell is **fish**, pass `--fish`
  (`zmx run dev --fish ls src`). Newer zmx builds run `run` commands under
  `/bin/bash` regardless of your login shell, for exactly this reason.
...
- **`references/commands.md`** — every subcommand: all flags, defaults, exit
  codes, examples, and per-command gotchas.
- **`references/recipes.md`** — parsing and filtering `zmx list` / `zmx version`
  output without a JSON mode: field extraction, filtering by client count or
  directory, feeding name lists into `kill` / `wait`, a readable table, and
