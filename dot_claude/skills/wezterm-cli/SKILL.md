...
- **`references/workspace-switching.md`** — switching the *active* workspace from the CLI. `wezterm cli` has no native command for it; covers the `SetUserVar` OSC + `user-var-changed` Lua bridge workaround, the must-emit-from-an-active-workspace-pane gotcha, and this machine's custom wiring (shell function, Lua handler, `notify.nim` integration).
