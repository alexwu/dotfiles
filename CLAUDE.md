...
- `scripts/claude/secret_guard.nim` is a PreToolUse Bash hook that gates commands referencing sensitive paths (SSH keys, `.env`, cloud creds, age/sops, rclone/ngrok/fnox/Copilot tokens, atuin sync key). Two-tier: always-deny files vs. sensitive dirs that pass inspection commands (ls/eza/tree/stat/…) unless chained with `|`, `;`, `&`, backtick, or `$(…)`
