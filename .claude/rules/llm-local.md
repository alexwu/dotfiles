...
- **`always-on`** (`persistent: true`, `swap: false`, `exclusive: false`) — small helpers that run concurrently and can't be evicted. Members: `sweepai/sweep-next-edit-1.5b`, `henrik3/sweep-next-edit-v2-7B`, `Qwen3.5-9B-MLX-4bit`, `zed-industries/zeta-2`, `zed-industries/zeta-2.1`.
- **`workhorse`** (`swap: true`, `exclusive: false`, `persistent: true`) — mid-tier models that hold the slot against default-group churn (zeta-2 / sweep / one-off models can't evict them) but trade among themselves. Members: `Qwen3.6-27B`, `Qwen3.6-35B-A3B`. Reserve the `heavy` group name for the true heavyweights (Mistral Medium 3.5 128B, Qwen3.5-122B-A10B) when those move out of the default group.
